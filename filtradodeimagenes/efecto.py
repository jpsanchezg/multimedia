...
if __name__ == '__main__':
    heya = Image(filename='heya.png')

    # let's apply a sobel edge detection kernel on the x and y axis
    sobel_x = apply_kernel(heya, np.array([[1, 2, 1], [0, 0, 0], [-1, -2, -1]]))
    sobel_y = apply_kernel(heya, np.array([[1, 0, -1], [2, 0, -2], [1, 0, -1]]))

    # let's combine these and make an edge detector!
    sobel_xy = combine_images(sobel_x, sobel_y)
    sobel_xy.write_image('heyaneon.png')
